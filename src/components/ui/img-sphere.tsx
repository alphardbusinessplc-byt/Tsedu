 ...
 *
 * Images arranged in a 3D sphere using Fibonacci distribution.
 * Drag-to-rotate, momentum physics, auto-rotation, and modal image viewing.
 *
 * This version adds robust tap-vs-drag detection so a click reliably opens
 * the enlarged modal even while the sphere is auto-rotating.
 */
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  // Track total drag distance to distinguish a tap from a drag
  ...
  // Open modal only when this was a tap, not a drag
  const handleImageActivate = useCallback((image: ImageData) => {
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      <div
        ...
        onClick={() => handleImageActivate(image)}
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/60">
          <img
            src={image.src}
            alt={image.alt}
  ...
  }, [worldPositions, baseImageSize, containerSize, hoveredIndex, handleImageActivate]);
      ...
      >
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-3 right-3 w-9 h-9 bg-black/50 rounded-full text-white flex items-center justify-center hover:bg-black/70 transition-all cursor-pointer"
              aria-label="Close"
            >
              <X size={18} />
            </button>
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6 text-center">
              {selectedImage.title && (
                <h3 className="text-xl font-semibold mb-1" style={{ color: '#8e2a4a' }}>{selectedImage.title}</h3>
              )}
  ...
  if (!isMounted) {
    return (
      <div
        ...
        style={{ width: containerSize, height: containerSize, background: 'var(--pink-soft)' }}
      >
        <div style={{ color: 'var(--rose)' }}>Loading...</div>
      </div>
    );
  }
  ...
  if (!images.length) {
    return (
      <div
        ...
        style={{ width: containerSize, height: containerSize, borderColor: 'var(--pink)' }}
      >
        <div className="text-center" style={{ color: 'var(--rose)' }}>
